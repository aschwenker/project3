...
this package takes the RSS XML and translates it into a dataframe where each tag is its own attribute. From this we reivewed the available attributes and chose to use hte item_content attribute for the word frequency. Note that the content of the RSS feed is only story summaries (truncated after approximately 150-180 characters), so this will limit our word counts.
...
#review observations of these attributes
data_science_skills$item_link
data_science_skills$item_content
```
Next we needed the source text to be in vector format, so we took the item_content column and converted it into a vector that would easily work with the tm package to tokenize the data.
...
The result of the word count processing is a `textcnt` data types, not a data frame.  We use the `ldply` function to transform `data1` into a single data frame of word counts, and then rename the columns so that we could create charts and graphs from the word frequency data frame. 
...
In this case, because the RSS feed only gave us 5 summaries of articles, our word counts are relatively low. We had hoped to be able to use the article links to mine the full text associated with each link, but this challenge proved to be larger than we were able to address in the time frame of this project. 

## Textbook

Likewise, we start by converting `data2` into a data frame of word counts so that we could create charts and graphs from the word frequency data frame. 
...
Because the RSS field was limited to a summary of the article we were limited to words that were within the beginning of each link which greatly reduces the usefulness of this source. Ideally we would have access to the full text and be able to make better inferences from that.

* other limitations?

# Conclusions
From the RSS feed frequency chart we were able to discern how data science skills are being spoken by others. A large part of staying relevant in a field is to keep on top of the topics and ideas that others in that field are talking about. 
...
Future analysis:
* mine many articles on data science and find a way to analyze this source to isolate those that are related to skills, data types or programs in order to bring more value from the frequency of the words. 

* Create a dashboard of word frequency from the RSS feed so that a user interested in keeping up to date on the subject could quickly see the topics being discussed by others.
